...
const Header = () => {
  return (
    <header className='py-4 px-3 flex items-center justify-center bg-primary'>
      <Container className='flex items-center justify-between notebook:justify-start notebook:gap-x-4'>
        <Link className='z-10 cursor-pointer' href='/'>
          <Image
            className='w-16 notebook:w-20'
            src={SenaiLogo}
            alt='Senai Logo'
            quality={100}
            priority
        ...
        <div className='hidden tablet:flex justify-between gap-x-12 notebook:gap-x-4'>
          {links.map((link) => (
            <Link
              key={link.route}
              href={link.route}
